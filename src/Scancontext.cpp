...
// Ring-Key的生成函数，提取的是行向量的均值，Scancontext每一行的均值作为Ringkey
...
std::pair<int, float> SCManager::detectLoopClosureID(void)
{
    ...
    /*
     *  step 2: pairwise distance (find optimal columnwise best-fit using cosine distance)
     计算候选Scan-Context的距离(相似度)，从中找到距离最近(相似度最高)的那个
     */
    TicTocV2 t_calc_dist;
    for (int candidate_iter_idx = 0; candidate_iter_idx < NUM_CANDIDATES_FROM_TREE; candidate_iter_idx++)
    {
...
// 重定位检测主函数
std::pair<int, float> SCManager::detectReLocalizationID(pcl::PointCloud<SCPointType> &curPointCloud)
{
    int loop_id{-1}; // init with -1, -1 means no loop (== LeGO-LOAM's variable "closestHistoryFrameID")
    Eigen::MatrixXd curr_desc = makeScancontext(curPointCloud); 
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext(curr_desc);
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext(curr_desc);
