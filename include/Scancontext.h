...
#include "common/KDTreeVectorOfVectorsAdaptor.h"

#include "common/tictoc.h"
...
public: 
    ...
    std::pair<int, float> detectReLocalizationID(pcl::PointCloud<SCPointType> &curPointCloud); // 重定位的函数
...
public:
    ...
    // tree
    const int    NUM_EXCLUDE_RECENT = 50; // simply just keyframe gap (related with loopClosureFrequency in yaml), but node position distance-based exclusion is ok. 
