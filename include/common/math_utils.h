...
namespace math
{
    ...
    inline bool esti_plane_dynamic(Eigen::Matrix<T, 4, 1> &abcd, const std::vector<Vec3d> &point, const double &threshold)
    {
        if (point.size() < 3)
        {
            std::cout << "ERROR!!!!!!!!!!the number of points should not be less than 3, given " << point.size() << std::endl;
            return false;
        }
        // Marked by xiaotao:
                           ...
                           T &best_match, float time_th = 0.5)
    {
        if (data.empty())
        {
            std::cout << "cannot interp because data is empty. " << std::endl;
            return false;
        }
                    ...
                    SE3 &result, T &best_match)
    {
        ...
        if (query_time > data.rbegin()->first)
        {
            std::cout << "query time is later than last, " << std::setprecision(18) << ", query: " << query_time
                      << ", end time: " << data.rbegin()->first << std::endl;
